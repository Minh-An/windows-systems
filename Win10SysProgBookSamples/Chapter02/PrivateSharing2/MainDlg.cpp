...
LRESULT CMainDlg::OnInitDialog(UINT /*uMsg*/, WPARAM /*wParam*/, LPARAM /*lParam*/, BOOL& /*bHandled*/) {
	...
	m_hBD.reset(::CreateBoundaryDescriptor(L"MyDescriptor", 0));
	if (!m_hBD)
	...
	m_hNamespace.reset(::CreatePrivateNamespace(nullptr, m_hBD.get(), L"MyNamespace"));
	if (!m_hNamespace) { // maybe created already?
		m_hNamespace.reset(::OpenPrivateNamespace(m_hBD.get(), L"MyNamespace"));
		if (!m_hNamespace)
...
LRESULT CMainDlg::OnRead(WORD, WORD, HWND, BOOL &) {
	wil::unique_mapview_ptr<void> buffer(::MapViewOfFile(m_hSharedMem.get(), FILE_MAP_READ, 0, 0, 0));
	if (!buffer) {
...
LRESULT CMainDlg::OnWrite(WORD, WORD, HWND, BOOL &) {
	wil::unique_mapview_ptr<void> buffer(::MapViewOfFile(m_hSharedMem.get(), FILE_MAP_WRITE, 0, 0, 0));
	if (!buffer) {
		AtlMessageBox(m_hWnd, L"Failed to map memory", IDR_MAINFRAME);
		return 0;
	}
	...
	::wcscpy_s((PWSTR)buffer.get(), text.GetLength() + 1, text);
